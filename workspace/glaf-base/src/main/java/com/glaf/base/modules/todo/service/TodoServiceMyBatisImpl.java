...
public class TodoServiceMyBatisImpl implements TodoService {
	...
	public List getTodoInstanceList(TodoQuery query) {
		...
		if (rows != null && rows.size() > 0) {
			...
			Collection<SysDeptRole> deptRoles = sysUserService
					.findByAccountWithAll(actorIdx).getDeptRoles();
			if (deptRoles != null && deptRoles.size() > 0) {
				Iterator iteratorxy = deptRoles.iterator();
	...
	public List getUserEntityList(String actorId) {
		...
		sb.append(
				" SELECT distinct d.userId actorId, d.depId deptId, c.id roleId ")
				.append(" FROM userrole a  ")
				.append(" INNER JOIN net_role c ON a.roleId = c.id ")
				.append(" INNER JOIN UserInfo d ON a.userId = d.userId ")
				.append(" WHERE ( d.userId = '").append(actorId).append("' ) ");
	...
	public Map getUserRoleMap(String actorId) {
		Map roleMap = new HashMap();
		Collection<SysDeptRole> deptRoles = sysUserService
				.findByAccountWithAll(actorId).getDeptRoles();
		if (deptRoles != null && deptRoles.size() > 0) {
			Iterator iterator = deptRoles.iterator();
			while (iterator.hasNext()) {
	...
	public Map getUserTodoMap(String actorId) {
		...
		if (deptRoles != null && deptRoles.size() > 0) {
			...
			while (iterator.hasNext()) {
				SysDeptRole sysDeptRole = (SysDeptRole) iterator.next();
				SysRole role = sysRoleService.findById(sysDeptRole.getRoleId());
				logger.debug("role.getCode():" + role.getCode());
