...
@Controller
@RequestMapping("/workspace/message")
public class MessageController {
	...
	public ModelAndView batchDelete(HttpServletRequest request,
		...
		if (id != null) {
			for (int i = 0; i < id.length; i++) {
				...
				// 判断是否是自己的消息
				if (bean == null || !StringUtils.equals( bean.getRecver().getActorId() ,user.getActorId()) ){
					ret = false;
				} else {
	...
	public ModelAndView indexList(ModelMap modelMap, HttpServletRequest request) {
		...
		com.glaf.core.util.PageResult messagePager = messageService
				.getNoReadList(user.getActorId(), new HashMap<String, Object>(), 1,
						msgPageSize);
	...
	public ModelAndView saveAndSend(HttpServletRequest request,
		...
		bean.setSender(user);
		bean.setSenderId(user.getActorId());
		bean.setCategory(0);// 收件箱
		bean.setReaded(0);
		bean.setCreateDate(new Date());

		int type = 1;// 用户消息
		if (bean.getSender().getActorId() == null
		...
		if (StringUtils.equals(messageType, "msg")
			...
			// 如果发送邮件，保存该信息即可
			bean.setRecver(user);
			bean.setRecverId(user.getActorId());
	 ...
	 * @return
	 */
	@RequestMapping("/showReceiveList")
