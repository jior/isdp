...
<script type="text/javascript">
	...
	function zTreeOnClick(event, treeId, treeNode, clickFlag) {
		...
		loadData('<%=request.getContextPath()%>/mx/branch/department/json?parentId='+treeNode.id);
	}
	...
	function editSelected(){
	    ...
	    if ( selected ){
		  var link = "<%=request.getContextPath()%>/mx/branch/department/prepareModify?id="+selected.id;
		  art.dialog.open(link, { height: 450, width: 680, title: "修改记录", lock: true, scrollbars:"yes" }, false);
	    }
	}
	...
	function deptRoles(){
	    ...
	    if ( selected && selected.id != null && selected.id != 'undefined'){
		  var link = "<%=request.getContextPath()%>/mx/branch/deptRole?deptId="+selected.id;
		  art.dialog.open(link, { height: 420, width: 680, title: "部门角色", lock: true, scrollbars:"yes" }, false);
	    }
	}

	function deptRoles2(deptId){
		var link = "<%=request.getContextPath()%>/mx/branch/deptRole?deptId="+deptId;
		art.dialog.open(link, { height: 420, width: 680, title: "部门角色", lock: true, scrollbars:"yes" }, false);
	}
	...
	function deptUsers(){
	    ...
	    var selected = jQuery('#mydatagrid').datagrid('getSelected');
	    if ( selected && selected.id != null && selected.id != 'undefined'){
		  var link = "<%=request.getContextPath()%>/mx/branch/user?deptId="+selected.id;
		  art.dialog.open(link, { height: 430, width: 880, title: "部门用户", lock: true, scrollbars:"yes" }, false);
	    }
	}

	function deptUsers2(deptId){
		var link = "<%=request.getContextPath()%>/mx/branch/user?deptId="+deptId;
		art.dialog.open(link, { height: 430, width: 880, title: "部门用户", lock: true, scrollbars:"yes" }, false);
	}
        ...
        <div class="easyui-layout" data-options="fit:true">  
           ...
           <div data-options="region:'center',split:true,border:true, fit:true">
			 ...
			 <table id="mydatagrid" class="easyui-datagrid" 
					data-options="url:'${contextPath}/mx/branch/department/json', fit:true,fitColumns:true,nowrap:false,rownumbers:false,showFooter:true,singleSelect:true,onDblClickRow:onMyDbClickRow">
				<thead>
					<tr>
