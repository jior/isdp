...
function uploadFile(){
  ...
  if(json==null || json==""){
	...
	//alert(json);
  	jQuery.ajax({
			type: "POST",
			url: '<%=request.getContextPath()%>/mx/others/attachment/save?referId=<%=referId%>&referType=<%=referType%>&json='+json,
			dataType:  'json',
				error: function(data){
					alert('服务器处理错误！');
				},
...
function openUpload(obj, type){  
	return ShowDialog('<%=request.getContextPath()%>/mx/others/attachment/showUpload' + (type ? '?type=' + type : ''), 450, 230, 'no', false, obj);
}
</script>
</head>

<body>
<html:form action="/mx/others/attachment/batchDelete" method="post">
...
<TR>
  <TD class="td-c"><span class="td-cb">
   ...
   <TD height="20" class="td-text" title="<%=bean.getName()%>"><a href="<%=request.getContextPath()%>/mx/others/attachment/download?referType=<%=referType%>&referId=<%=referId%>&id=<%=bean.getId()%>"><%=bean.getName()%></a></TD>
