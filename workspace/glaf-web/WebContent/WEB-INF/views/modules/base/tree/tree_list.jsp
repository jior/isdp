...
function sort(id, operate){  
  //SysTreeAjaxService.sort(<%=parent%>, id, operate, {callback:function (){reloadPage();}});
         jQuery.ajax({
			type: "POST",
			url: '<%=request.getContextPath()%>/mx/base/tree/sort?parent=<%=parent%>&id='+id+'&operate='+operate,
			dataType:  'json',
				error: function(data){
					alert('服务器处理错误！');
				},
...
<html:form action="${contextPath}/mx/base/tree/batchDelete" method="post" target="_self"> 
