...
public class ReceiveMailBean {
	...
	public ReceiveMailBean(MimeMessage mimeMessage, MailRec mailRec) {
	...
	public void getMailContent(Part part) throws MessagingException,
		...
		if (part.isMimeType("text/plain") && !conname) {
			mailRec.setText((String) part.getContent());
		} else if (part.isMimeType("text/html") && !conname) {
			mailRec.setHtml((String) part.getContent());
	...
	public void receive(Part part, int i) throws MessagingException,
			IOException {
		logger.debug("------------------START-----------------------");
		mailRec.setMsgId(this.getMessageId());
		mailRec.setDate(mimeMessage.getSentDate());
		mailRec.setSubject(this.getSubject());
		mailRec.setFrom(this.getFrom());
		mailRec.setTo(this.getMailAddress("TO"));
		mailRec.setCc(this.getMailAddress("CC"));
	...
	public void receiveMail(String popServer, int port, String username,
		...
		try {
			...
			MailRec mailRec = null;
			IMailRecService mailRecService = ContextFactory
					.getBean("mailRecService");
			logger.debug(mailRecService.nextId());
			for (int i = 0; i < count; i++) {
				MimeMessage msg = (MimeMessage) msgs[i];
				if (mailRecService.getMailRecByMessageId(msg.getMessageID()) == null) {
					try {
						mailRec = new MailRec();
						mailRec.setEmail(username);
						this.setMimeMessage(msg);
						this.setMailRec(mailRec);
						if (attachSavePath != null) {
							...
							this.setAttchSavePath(attachSavePath + FileUtils.sp
									+ mailRec.getEmail() + FileUtils.sp
									+ hex_str);
						}
						this.receive(msgs[i], i);
						mailRecService.save(mailRec);
					} catch (Exception ex) {
						ex.printStackTrace();
					} finally {
						mailRec = null;
	...
	public void saveFile(String filename, InputStream inputStream) {
		...
		try {
			...
			MailRecAtt att = new MailRecAtt();
			att.setCtime(new Date());
			att.setFileContent(bytes);
			att.setFileName(filename);
			att.setFilesize(bytes.length);
	...
	public void setMailRec(MailRec mailRec) {
		this.mailRec = mailRec;
	}
